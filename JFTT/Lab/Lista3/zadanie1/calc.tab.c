...
#line 45 "calc.y"
          {
          /* Liczba przychodzi z Lexera już znormalizowana (nawet jeśli była ujemna) */
          yyval = normalize(yyvsp[0]);
          printf("%lld ", yyval);
      }
...
#line 75 "calc.y"
                              {
          /* Ta reguła zadziała teraz tylko dla wyrażeń w nawiasach, np. -(2+2) */
          /* Stałe ujemne np. -2 są wyłapywane przez Lexer jako NUM */
          yyval = sub(0, yyvsp[0]);
          printf("NEG ");
      }
...
long long add(long long a, long long b) { return normalize(a + b); }
long long sub(long long a, long long b) { return normalize(a - b); }
long long mul(long long a, long long b) { return normalize(a * b); }
...
long long modInverse(long long n) {
    ...
    long long r = P, newr = normalize(n);
    while (newr != 0) {
...
void yyerror(const char *s) { fprintf(stderr, "Błąd: %s\n", s); }
